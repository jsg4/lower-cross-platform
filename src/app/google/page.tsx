...
import { ChevronDown, ChevronRight, Chrome } from 'lucide-react'
...
export default function GoogleAdsPage() {
  ...
  const [adGroups, setAdGroups] = React.useState<Record<string, AdGroupData[]>>({})
  const [loading, setLoading] = React.useState(true)
  const [loadingAdGroups, setLoadingAdGroups] = React.useState<string | null>(null)
  ...
  React.useEffect(() => {
    async function loadData() {
      ...
      setLoading(true)
      try {
        const data = await getCampaignPerformance(selectedClientId, 'google', dateRange)
        setCampaigns(data)
      } finally {
        setLoading(false)
      }
  ...
  const handleExpandCampaign = async (campaignId: string) => {
    ...
    if (!adGroups[campaignId] && selectedClientId) {
      ...
      <div className="flex h-full items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-zinc-400">
            Select a client from the dropdown to view Google Ads data.
          </p>
        </div>
      </div>
    )
              ...
              <TableBody>
                {campaigns.map((campaign) => (
                  <React.Fragment key={campaign.campaignId}>
                    ...
                    {expandedCampaign === campaign.campaignId && (
                      <>
                        {loadingAdGroups === campaign.campaignId ? (
                          ...
                          adGroups[campaign.campaignId || '']?.map((adGroup) => (
                            ...
                            >
                              ...
                              <TableCell className="text-right text-zinc-400">
                                {formatCurrency(adGroup.revenue)}
                              </TableCell>
                              <TableCell className="text-right">
                                <span
                                  className={cn(
                              ...
                              <TableCell className="text-right text-zinc-400">
                                ${adGroup.cpm.toFixed(2)}
                              </TableCell>
                              <TableCell className="text-right text-zinc-400">
                                {adGroup.ctr.toFixed(2)}%
                              </TableCell>
                              <TableCell className="text-right text-zinc-400">
                                {adGroup.cvr.toFixed(2)}%
                              </TableCell>
                              <TableCell>
                                <Sparkline data={generateSparkline()} />
                              </TableCell>
